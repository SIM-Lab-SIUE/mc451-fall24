...
In `ggplot2`, creating a plot involves three primary components:
1. **Data**: The dataset you want to visualize.
2. **Aesthetic Mappings (aes)**: How the variables in your data relate to the visual properties of the plot, such as the x-axis, y-axis, color, size, or shape.
3. **Geometries (geom)**: The type of plot you want to create, like a bar chart, line graph, or scatter plot.
...
In this example:
- `swift_songs` is the dataset, where `album_name` is the listed album name, and `duration_hrs` is the collective time of the tracks on the ablum.
- `geom_bar(stat = "identity")` specifies a bar chart, where the height of each bar corresponds directly to the `shares` value for each day.
