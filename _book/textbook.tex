...
\addcontentsline{toc}{subsection}{Why Use a Chi-Square Test?}
...
\addcontentsline{toc}{subsection}{Chi-Square Test in R}
...
\NormalTok{horror\_movies }\OtherTok{\textless{}{-}}\NormalTok{ horror\_movies }\SpecialCharTok{\%\textgreater{}\%}
...
\NormalTok{table\_genre\_ratings }\OtherTok{\textless{}{-}} \FunctionTok{table}\NormalTok{(horror\_movies}\SpecialCharTok{$}\NormalTok{genre\_names, horror\_movies}\SpecialCharTok{$}\NormalTok{vote\_average\_category)}
...
\tightlist
\item
  \textbf{Warning: Chi-squared approximation may be incorrect}: This warning often occurs when the expected frequencies in some cells of the contingency table are too small (less than 5), which can lead to unreliable results.
\item
  \textbf{X-squared = NaN}: This indicates that the test failed to compute the chi-square statistic, likely due to empty cells or categories with very few observations.
\item
  \textbf{p-value = NA}: The p-value is not available, meaning the test could not produce valid results. You may need to reduce the number of categories or ensure that every category has sufficient data before running the test again.
\end{itemize}

\subsection*{Interpreting Chi-Square Output}\label{interpreting-chi-square-output}
\addcontentsline{toc}{subsection}{Interpreting Chi-Square Output}
...
\addcontentsline{toc}{subsubsection}{T-test in R}

We can use \texttt{dplyr} to filter the data for these two genres and then run a t-test using the \texttt{t.test()} function.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Filter for horror and comedy movies}
\NormalTok{horror\_comedy }\OtherTok{\textless{}{-}}\NormalTok{ horror\_movies }\SpecialCharTok{\%\textgreater{}\%}
...
\NormalTok{t\_test\_result }\OtherTok{\textless{}{-}} \FunctionTok{t.test}\NormalTok{(vote\_average }\SpecialCharTok{\textasciitilde{}}\NormalTok{ genre\_names, }\AttributeTok{data =}\NormalTok{ horror\_comedy)}
...
\addcontentsline{toc}{subsection}{Interpreting T-test Output}
...
A \textbf{paired samples t-test} is used when comparing the means of the same group at two different points in time or under two different conditions. For example, if we wanted to compare the average viewer ratings of movies at the start and end of a movie-watching experiment, we could use a paired t-test.

\subsubsection*{Paired T-test Example}\label{paired-t-test-example}
\addcontentsline{toc}{subsubsection}{Paired T-test Example}

Assume that we want to compare the \texttt{vote\_average} of a set of horror movies between two conditions: ``before watching a trailer'' and ``after watching a trailer.''

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create a paired dataset (hypothetical example)}
\NormalTok{before\_trailer }\OtherTok{\textless{}{-}}\NormalTok{ horror\_movies }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{filter}\NormalTok{(genre\_names }\SpecialCharTok{==} \StringTok{"Horror"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{mutate}\NormalTok{(}\AttributeTok{condition =} \StringTok{"Before Trailer"}\NormalTok{)}
\NormalTok{after\_trailer }\OtherTok{\textless{}{-}}\NormalTok{ horror\_movies }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{filter}\NormalTok{(genre\_names }\SpecialCharTok{==} \StringTok{"Horror"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{mutate}\NormalTok{(}\AttributeTok{condition =} \StringTok{"After Trailer"}\NormalTok{)}

\CommentTok{\# Join the datasets for a paired t{-}test (matching on movie title)}
\NormalTok{paired\_data }\OtherTok{\textless{}{-}}\NormalTok{ before\_trailer }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{rename}\NormalTok{(}\AttributeTok{vote\_before =}\NormalTok{ vote\_average) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{inner\_join}\NormalTok{(after\_trailer }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{rename}\NormalTok{(}\AttributeTok{vote\_after =}\NormalTok{ vote\_average), }\AttributeTok{by =} \StringTok{"title"}\NormalTok{)}

\CommentTok{\# Perform paired t{-}test}
\NormalTok{paired\_t\_test\_result }\OtherTok{\textless{}{-}} \FunctionTok{t.test}\NormalTok{(paired\_data}\SpecialCharTok{$}\NormalTok{vote\_before, paired\_data}\SpecialCharTok{$}\NormalTok{vote\_after, }\AttributeTok{paired =} \ConstantTok{TRUE}\NormalTok{)}
...
\tightlist
\item
  \textbf{Warning: Many-to-many relationship}: This suggests that the same movie may appear more than once in either the ``before'' or ``after'' dataset. Ensure that each movie is unique for a valid paired t-test.
\item
  \textbf{t = 0}: The t-statistic is 0, meaning there is no difference between the paired observations.
\item
  \textbf{p-value = 1}: A p-value of 1 means there is no evidence to reject the null hypothesis, indicating no significant difference between the two conditions.
\item
  \textbf{Mean difference = 0}: The average difference between the paired scores is 0, confirming no change between the two conditions.
\end{itemize}

\subsection*{Interpreting Paired T-test Output}\label{interpreting-paired-t-test-output}
\addcontentsline{toc}{subsection}{Interpreting Paired T-test Output}
...
\addcontentsline{toc}{subsubsection}{ANOVA in R}
...
\addcontentsline{toc}{subsection}{Interpreting ANOVA Output}
...
\addcontentsline{toc}{subsubsection}{Linear Regression in R}
...
\addcontentsline{toc}{subsection}{Cohen's d for T-tests}
...
\addcontentsline{toc}{subsubsection}{Calculating Cohen's d in R}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Calculate Cohen\textquotesingle{}s d for horror and comedy movies}
\NormalTok{cohen\_d\_result }\OtherTok{\textless{}{-}} \FunctionTok{cohen.d}\NormalTok{(vote\_average }\SpecialCharTok{\textasciitilde{}}\NormalTok{ genre\_names, }\AttributeTok{data =}\NormalTok{ horror\_comedy)}
...
\addcontentsline{toc}{subsection}{Interpreting Cohen's d}
...
\addcontentsline{toc}{subsection}{\(R^2\) for Regression Analysis}
...
\addcontentsline{toc}{subsubsection}{Interpreting \(R^2\)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Fit a regression model to predict score based on episodes}
\NormalTok{linear\_model }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(score }\SpecialCharTok{\textasciitilde{}}\NormalTok{ episodes, }\AttributeTok{data =}\NormalTok{ anime)}

\CommentTok{\# Extract and print the R{-}squared value}
\FunctionTok{summary}\NormalTok{(linear\_model)}\SpecialCharTok{$}\NormalTok{r.squared}
\end{Highlighting}
\end{Shaded}
...
\addcontentsline{toc}{subsection}{Effect Size for ANOVA: Eta-Squared (\(\eta^2\))}
...
\addcontentsline{toc}{subsubsection}{Calculating \(\eta^2\) in R}
...
\addcontentsline{toc}{subsubsection}{Interpreting \(\eta^2\)}
...
\addcontentsline{toc}{subsection}{Simple Logistic Regression}
...
\addcontentsline{toc}{subsection}{Interpreting Logistic Regression Output}
...
\addcontentsline{toc}{subsection}{Multiple Logistic Regression}
...
\addcontentsline{toc}{subsection}{Interpreting Multiple Logistic Regression Output}
...
\addcontentsline{toc}{subsubsection}{Calculating Confidence Intervals in R}
...
\addcontentsline{toc}{subsection}{Example: Testing Whether Genre Affects Viewer Ratings}
